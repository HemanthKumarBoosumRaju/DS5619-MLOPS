...
def expect_column_positive(rows, column):
    ...
    for i, row in enumerate(rows):
        rowData=row[column]
        try:
            floatnumber = float(rowData)
        except (TypeError, ValueError):
            violations.append(
                Violation(
                    expectation="expect_value_can_be_cast_into_float",
                    column=column,
                    row_index=i,
                    detail="value cant be cast into float"
                )
            )
            continue
        ...
        if floatnumber<=0:
            violations.append(
                Violation(
                    expectation="expect_column_is_positive",
                    column=column,
                    row_index=i,
                    detail="value is not positive"
                )
            )
...
def expect_column_in_set(rows, column, allowed_values):
    ...
    for i, row in enumerate(rows):
        rowData =row[column]
        if rowData not in allowed_values:
            violations.append(
                Violation(
                    expectation="expect_column_in_allowed_values",
                    column=column,
                    row_index=i,
                    detail="value is not in allowed_values"
                )
            )
...
def expect_column_unique(rows, column):
    ...
    for i, row in enumerate(rows):
        ...
        if rowData in seen:
            violations.append(
                Violation(
                    expectation="expect_column_to_be_unique",
                    column=column,
                    row_index=i,
